...
%% assume the epoches index has been arranged properly
...
for ii = 1:1:n_cell
    ...
    elseif strcmp(mode, 'sub_norm_by_sum')
%         resp_reshape(resp_reshape <= 0) = 0;
        resp_reshape_ave = -diff(resp_reshape,1,which_dim + 2)./sum(resp_reshape, which_dim + 2); %% (first - second)/(first + second);
    end
