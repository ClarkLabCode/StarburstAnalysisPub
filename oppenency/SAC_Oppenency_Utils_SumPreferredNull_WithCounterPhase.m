...
for pp = 1:1:n_phase
    for dd = 1:1:n_dir
        ...
        SAC_Plot_Utils_PlotTimeTraces(resp_ave(:, epoch_index(pp, dd)), resp_sem(:, epoch_index(pp, dd)), color_use,...
            'set_integrate_len',true,'f_vals', 4);
    end
    subplot(2, ceil(n_phase/2), pp);
    SAC_Plot_Utils_PlotTimeTraces(resp_ave_pref_plus_null(:,pp), resp_sem_pref_plus_null(:, pp), color_sum, 'set_integrate_len',true,'f_vals', 4);
    set(gca,'YLim',[-0.75, 1.5]);
    xlabel('time (t)');
    title(['phase:', num2str(pp)]);
    if pp == 1
        legend('preferred','null','counterphase', 'linear prediction: pref+null');
    end
end
end
