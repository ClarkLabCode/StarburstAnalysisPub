function SAC_AppMot_Plot_Utils_4D_BarPlot_Lag_Dir_LeadC_LagC(resp, data_info, stim_info, epoch_ID, recording_info, plot_max_value,figure_name)
...
%% average response overl recordings.
[resp_ave_ind, resp_ave, resp_sem] = SAC_GetAverageResponse(resp_over_time);
[resp_over_X, data_info_over_X, ~] = SAC_AverageResponse_By(resp_over_time, data_info,'lagCont','sub',epoch_ID);
...
for cc_lead = 1:1:2 %  Lef and Right Pannel.
    for dd = 1:1:2
        ...
        %% plot stimulus.
        block_row = 1; % first row.
        block_mega_col = dd;
        block_smaller_col = cc_lead;
        block_idx = (block_row - 1) * 4 + (block_mega_col - 1) * 2 + block_smaller_col;
        subplot_idx_two_resp  = 1 * 4 + block_idx ;
        subplot_idx_diff_resp = 2 * 4 + block_idx;
        
        for cc_lag = 1:1:2
            epoch_this = epoch_index(cc_lead, cc_lag, dd, 1);
        ...
        %% plot responses before subtraction
        for cc_lag = 1:1:2
            xaxis_this = x_axis_bank(cc_lag, :);
            [resp_ave_this, resp_sem_this, resp_ind_this, color_this] = ...
                SAC_AppMot_Plot_Utils_BarPlot_GetResponsesAtDifferentPhases(resp_ave, resp_sem, resp_ave_ind, epoch_index(cc_lead, cc_lag, dd,:), color_bank(cc_lag, :));
            subplot(3, 4, subplot_idx_two_resp);
        ...
        SAC_Plot_Utils_BarPlot_Setup_Axis(x_lim_plot, [-0.1, plot_max_value], x_axis, 0);
        
        %% plot responses after subtraction combination.
        [resp_ave_over_X_this, resp_sem_over_X_this, resp_ind_over_X_this, color_over_X_this] = ...
            SAC_AppMot_Plot_Utils_BarPlot_GetResponsesAtDifferentPhases(resp_over_X_ave, resp_over_X_sem, resp_over_X_ave_ind, epoch_index_over_X(cc_lead, dd, :), [0,0,0]);
        subplot(3, 4, subplot_idx_diff_resp);
        bar_scatter_plot_Juyue(resp_ave_over_X_this, resp_sem_over_X_this, resp_ind_over_X_this', color_over_X_this,...,
            'link_dots_flag', 0, 'xaxis', x_axis, 'bar_width', bar_width,...
            'plot_individual_dot', 0, 'do_sig_test_flag', 1, 'plot_max_value',  plot_max_value);
        SAC_Plot_Utils_BarPlot_Setup_Axis(x_lim_plot, [-0.1, plot_max_value], x_axis, 1)
