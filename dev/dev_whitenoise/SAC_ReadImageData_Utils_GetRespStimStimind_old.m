...
data_folder  = S.sac_data_path;
respfolder = fullfile(data_folder, cell_name);
stimtime_file = fullfile(respfolder, [cell_name(5:end),'.mat']);
datainfo = dir(fullfile(data_folder, cell_name, [cell_name, '_preproc','_f',num2str(fpass * 100),'.mat']));
datafile = fullfile(data_folder, cell_name, datainfo.name);
data= load(datafile);
