...
%         fprintf('%%NaN = %f\n', percNan);
        ...
        if avgTrialFirst
            ...
            averagedTrial = ReduceDimension(droppedNaNTraces, 'time', @nanmean);
            ...
            %% average over trials
            reducedTimesOut{ff} = ReduceDimension(averagedTrial,'trials',roiTimeReduceFunc);
        end
...
%     finalTitle = [fTitle ': ' epochsForSelectivity{1} ' - ' epochsForSelectivity{2}];
    ...
    if makePlots
        ...
        % Here we make the assumption that the plots go T4 Prog, T4 Reg, T5
        % Prog, T5 Reg, which is fine for this function as it's for
        % plotting stuff for the paper...
        if iteration == 1 || iteration == 3
            newPlot = true; %placeholder?
        else
            newPlot = false;
        end
    end
    ...
    if ~isempty(flyEyes)
        ...
        if makePlots
            ...
            % Here we make the assumption that the plots go T4 Prog, T4
            % Reg, T5 Prog, T5 Reg, which is fine for this function as it's
            % for plotting stuff for the paper...
            firstIfStatement = iteration == 1 || iteration == 3;
            if firstIfStatement
                plotOrderChange = true;
            end
        end
